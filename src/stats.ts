...
export default class Stats {
	...
	private handleActivity() {
		...
		if (this.lastEventTime && now - this.lastEventTime <= 5000) {
			...
			if (this.writingStartTime === null) {
				...
				this.initialCharCount = this.getCharCount(editor.document.getText());
			}
		...
		this.inactivityTimer = setTimeout(() => {
			...
			if (editor2) {
				...
				if (totalTime > 0) {
					// 速度 = 新增字数 / 总写作时长
					const added = count - this.initialCharCount;
					this.frozenSpeed = Math.max(0, added) / (totalTime / 1000);
				}
	...
	private update() {
		...
		// 速度 = 新增字数 / 总写作时长
		let speedStr = '—';
		if (totalWritingTime > 0 && this.writingStartTime !== null) {
			const added = total - this.initialCharCount;
			if (added > 0) {
				const charsPerSecond = added / (totalWritingTime / 1000);
